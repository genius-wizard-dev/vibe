...
function resolveResearchNewTarget(args) {
  let topic = "";
  ...
  if (topicFromFlag && positional.length > 1) {
    throw new Error("Too many arguments for research new");
  }
...
function createResearchTopic(baseDir, topic, scope) {
  const updatedAt = new Date().toISOString();
  const topicDir = path.join(baseDir, topic);

  if (fs.existsSync(topicDir)) {
    throw new Error(`Research topic already exists: ${shortenHome(topicDir)}`);
  }
...
`,
  ...
  ensureFile(path.join(topicDir, "brief.md"), `# ${topic} brief\n\n`);
  ensureFile(path.join(topicDir, "interview.md"), `# ${topic} interview\n\n`);
  ensureFile(path.join(topicDir, "analysis.md"), `# ${topic} analysis\n\n`);
  ensureFile(path.join(topicDir, "discussion.md"), `# ${topic} discussion\n\n`);
  ...
  appendOverviewTopic(path.join(baseDir, "overview.md"), topic, "scan", updatedAt);
...
Usage:
  ...
  vibe research new --name <topic> [--root <project-root>] [--global]
...
async function runResearchNew(args) {
  try {
    const target = resolveResearchNewTarget(args);
    const slug = toSlug(target.topic || "");

    if (!slug) {
      throw new Error("Missing research topic name");
    }

    const baseDir = target.global
      ? path.join(os.homedir(), ".config", "vibe", "research")
      : path.join(path.resolve(process.cwd(), target.root), ".vibe", "research");

    const topicDir = createResearchTopic(
      baseDir,
      slug,
      target.global ? "global" : "local",
    );

    console.log(chalk.green(`\nCreated research topic: ${slug}`));
    console.log(chalk.dim(`  ${shortenHome(topicDir)}`));
    console.log(chalk.dim(`  Next: /research.scan\n`));
  } catch (error) {
...
/**
 * Routes `vibe research ...` subcommands.
 */
