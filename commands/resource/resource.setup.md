...
When repository already has a working codebase, prefer context-only setup:

- do not regenerate full project skeleton
- do not rewrite existing src/app/services structure
- use research/design outputs to refine docs, bridge context, and workflow decisions
...
Detect repository baseline before continuing:

- if source folders or app entry files already exist, mark `codebase_state: existing`
- if repository is mostly empty scaffold, mark `codebase_state: empty`

Baseline signals (examples):

- existing: `src/`, `app/`, `services/`, `backend/`, `frontend/`, `package.json`, `pyproject.toml`, `go.mod`
- empty: only docs/config, no runnable source modules

For `codebase_state: existing`, default behavior is `refine-only`:

- setup resource context from current codebase + research + design
- skip full scaffold generation unless user explicitly asks for it
...
- if `codebase_state: empty` -> run `/resource.base` to scaffold baseline
- if `codebase_state: existing` -> run `/resource.base` in refine-only mode (no structural overwrite)

Refine-only mode should only:

- align architecture boundaries with current folders/modules
- add missing non-destructive support files (docs/templates/config notes)
- propose structural changes as recommendations first, not direct rewrites
